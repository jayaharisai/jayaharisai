...
// Only used in dev mode. In production (static export), data comes from
// public/pages-data.json (generated at build time).
...
// GET /api/pages — fetch all published pages
...
// POST /api/pages — publish a new page
export async function POST(req: NextRequest) {
  try {
    ...
    // Read existing pages, add new one, write back
    const raw = await fs.readFile(DATA_FILE, "utf-8");
    const pages: PageData[] = JSON.parse(raw);
    ...
    if (existing >= 0) {
      pages[existing] = newPage; // overwrite
    } else {
      pages.unshift(newPage); // newest first
    }
